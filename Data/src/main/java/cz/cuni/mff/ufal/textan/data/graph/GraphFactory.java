...
public class GraphFactory {
    ...
    private final Logger LOG = LoggerFactory.getLogger(GraphFactory.class);
    ...
    public Graph getShortestPathBetweenObjects(ObjectTable obj1, ObjectTable obj2, int maxDepth) throws PathDoesNotExistException {
        LOG.debug("Object1: {}", obj1);
        LOG.debug("Object2: {}", obj2);
        ...
        for (int i = 0; i <= (maxDepth + 1) / 2; i++) {
            ...
            if (found) break;
            if (i == (maxDepth + 1) / 2) {
                LOG.debug("Graph1: {}", graph1);
                LOG.debug("Graph2: {}", graph2);

                LOG.debug("objQueue1: {}", objQueue1);
                LOG.debug("objQueue2: {}", objQueue2);
