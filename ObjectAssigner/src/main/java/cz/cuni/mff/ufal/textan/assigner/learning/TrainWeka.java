...
public class TrainWeka {
    ...
    public Classifier doTraining(final IObjectTableDAO objectTableDAO,
        ...
        for (ObjectTable obt : objectTable) {
            ...
            final String first_alias_text = als.get(0); //first_alias.getAlias();

            // Create a fake entity for training yes
            Entity e = new Entity(first_alias_text, type);
            // Create another fake entity for training no
            Entity e_reverse = new Entity(new StringBuilder(first_alias_text).reverse().toString(), type - 1);
    ...
    public Instance CreateInstanceBasic
        ...
        // Instance
        final Instance thisInstance = new Instance(5);
