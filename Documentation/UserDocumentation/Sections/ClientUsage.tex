...
The Dials \comment{Ondrej}{Proc dials?} menu contains items to display objects,
...
If the document is processed by someone else when you are processing it, an
...
\begin{figure}[!htb]
        ...
        \caption{Menu for selecting object. 1) Filter field, 2) List of
		 ...
		 during editing of the same type\comment{Ondrej}{report??}, 4) Button for creating new object,
...
\subsection{Errors \comment{Ondrej}{Conflict Resolution as the title and use conflict
instead of error everywhere??}}
\label{sssec:Errors}
...
This window contains the list of documents matching filter criteria. Please note
