...
and 64 bit) and Windows (32 bit and 64 bit). \comment{Petr}{Add note about
processor architecture or linux binaries are independent on it?} If you need to
...
Furthermore you need to edit properties used to connect to the database (\emph{jdbc}) and to generate sql queries by Hibernate (\emph{hibernate.dialect}). Their meaning is described in section \ref{sssec:DataSettings}.

\comment[Venca]{Petr}{Change of  the underlying database is not so easy, you
need to add db driver into classpath! Is it somewhere in the documentation? }
...
\comment{Petr}{These settings are related to Persistence layer - it means also indexing}
...
\comment[Kuba]{Adam}{May add an example here? I am still not sure about the
format of I-type and B-type}
