...
Usage of webservice interface \emph{IDataProvider} is straightforward, its
methods are completely independent and are intended for viewing and manipulating
the database.

The interface \emph{IDocumentProcessor} is more demanding and needs its methods
...
following calls. For three recognition methods there are two variants. First for
processing report already stored in the database (called\emph{*ById}) and second
for inserting completely new document to the database (called
\emph{*FromString}). Functions \emph{getEntities*} recognize entities in
given report. Functions \emph{getAssignments*} assign objects from the database
to recognized entities and finally functions \emph{getRelations*} recognize
...
The recognition functions do not store anything into the database. For this
purpose there are three methods \emph{saveProcessedDocumentFromString} and
\emph{saveProcessedDocumentById} with similar difference as meantioned above.
The last function \emph{rewriteAndSaveProcessedDocumentById} has as input
...
force returning to the original version or provide new one. All three methods
...
recognizing/assignments if needed, save methods can be called with \emph{force}
parameter set to \emph{true}.

\comment[Peter]{Adam}{Is there any place with documentation of webservice interface and individual methods, like javadoc or something?}
