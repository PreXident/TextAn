...
\comment[Petr]{Venca}{Right after all my bullshits}
...
it is appreciated when the persistence layer changes. For example in case of changing the underlaying database or Hibernate ORM, it is not necessary to remake the whole
...
To provide the data to application (\textan{} server in our case) \comment[Venca]{Petr}{What about upper layers or \textan server?}
\comment{Venca}{Like this?} it is used the DAO
...
\paragraph{}
\comment{Venca}{hibernate search and lucene ?}
...
object is a human or an address. It has only one meaningful column with name of the type and the types have to be
unique.
